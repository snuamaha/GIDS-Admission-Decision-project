    ...
    for i, file_name in enumerate(all_files):
        if file_name.endswith(".xlsx"):
            try:
                ...
                if (
                    "Rating" in r1.columns
                    and "Name" in r2.columns
                    and "Institution 1 GPA (4.0 Scale)" in r2.columns
                ):
...
def merge_reviewer_ratings1(file_list):
    ...
    for i, df in enumerate(file_list):
        ...
        name_df = pd.merge(
            name_df,
            df[["Applicant_Name", f"reviewer {i+1} Rating", f"Reviewer_Name_{i+1}"]],
            on="Applicant_Name",
            how="left",
        )
...
def make_decision_matrix2(decision_df):
    ...
    decision_matrix = pd.DataFrame(
        columns=["Applicant_Name", "Ratings", "Reviewers", "Decision"]
    )
